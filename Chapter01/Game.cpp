...
void Game::UpdateGame()
{
    ...
    if (mPaddleDir2 != 0)
    {
        ...
        else if (mPaddlePos2.y > (((float)windowH) - paddleH / 2.0f - thickness))
        {
            mPaddlePos2.y = ((float)windowH) - paddleH / 2.0f - thickness;
        }
    }
    ...
    for (auto& ball : mBalls) {
        ...
        if (
            ...
            diff2 <= paddleH / 2.0f &&
            // We are in the correct x-position
            ball.pos.x <= (float)(windowW-25.0f) && ball.pos.x >= (float)(windowW-30.0f) &&
            // The ball is moving to the left
            ball.vel.x > 0.0f)
        {
            ball.vel.x *= -1.0f;
	    ...
	    else if (ball.pos.x <= 0.0f || ball.pos.x >= (float)windowW)
	    {
		    mIsRunning = false;
	    }
	    ...
	    else if (ball.pos.y >= (windowH - thickness) &&
		    ball.vel.y > 0.0f)
	    {
		    ball.vel.y *= -1;
	    }
